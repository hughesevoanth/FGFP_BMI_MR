obsmr = function( wdata,
  ...
  ## if the outcome is binomial make sure the outcome is a factor
  if(family == "binomial"){
  ...
  if(family == "binomial"){
    W_outcome = NA; names(W_outcome) = "W_outcome"
  }
  ...
  ############################################
  if(family == "gaussian" & rnt_outcome == TRUE){
  ...
  ######################
  ### VII. IVreg model REPLACE THIS WITH IVTOOLS !!!
  ######################
