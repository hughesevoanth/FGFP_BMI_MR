lmfit_v3 = function( wdata,
  ...
  if(family == "binomial"){
    if( is.na(weights) ){
      glm_mod = glm(form, data = wdata, family = binomial() )
    } else {
      ## re-name weights column
      w = grep( weights, colnames(wdata) ); colnames(wdata)[w] = "study_weights"
      glm_mod = glm(form, weights = study_weights, data = wdata,  family = binomial() )  
    }
  }
  ...
  anova_P = a[exposure,6]
  names(anova_P) = "devexp_exposure_P"
  ...
  # sandwich_estimates = sandwich_estimates[-1]
  # 
  ######################
  ...
  glm_out = c(exposure, outcome, n, Wstat, Breusch_Pagan_P, devexp_by_model, devexp_by_exposure, anova_P, glm_estimates ) 
  # glm_out = c(exposure, outcome, n, Wstat, Breusch_Pagan_P, devexp_by_model, devexp_by_exposure, anova_P ) 
